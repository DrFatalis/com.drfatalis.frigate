...
const bird = (fill = WHITE) => `<g transform="translate(0,256) scale(0.1,-0.1)" fill="${fill}"><path d="${BIRD_PATH}"/></g>`;
// ─── SVG builders ──────────────────────────────────────────────────────────
/**
...
function render(svgString, outPath, w, h) {
    ...
    const resvg = new resvg_js_1.Resvg(svgString, { fitTo: { mode: 'width', value: w } });
    const png = resvg.render().asPng();
    (0, fs_1.writeFileSync)(outPath, png);
